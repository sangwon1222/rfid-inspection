...
const inspectStart = async () => await serialManager.inspectStart()

const calcSecond = (e) => {
  const target = e.currentTarget as HTMLInputElement
  store.idro.timeout = +target.value
}
...
<template>
  <div class="relative flex flex-wrap flex-col gap-4 px-6 w-full h-100">
    <div class="flex flex-wrap items-center gap-4 w-full h-60">
      <a-button
    ...
    <div class="flex gap-4">
      <div>
        ...
        <input
          type="number"
          class="w-100 text-xl"
          :value="store.idro.timeout"
          @input="calcSecond"
        />
      </div>
      <p>{{ store.idro.timeout / 1000 }} 초</p>
    </div>
  </div>
</template>
